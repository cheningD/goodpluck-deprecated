...
import { FieldWrapper, Header2, StyledErrorMessage, StyledField } from '../components/StyledComponentLib'
import React, { useState } from 'react'
import { VALID_ZIP_PATTERN, removeNonLetters } from '../util'
    ...
    shoppingForQuiz: {
      component: (
        <QuizForm
          ...
          items={['Myself', 'Spouse or Partner', 'Kids', 'Babies', 'Extended Family', 'Friends or Roommates', 'Pets']}
          percentComplete={20}
        />
      ),
...
const EmailZipForm = ({ onSubmit, header, percentComplete, goBackFunction, submitText }: EmailZipFormProps) => {
...
const QuizForm = ({ onSubmit, header, items, percentComplete, goBackFunction, blurb, submitText }: QuizFormProps) => {
  const initialValues = {}
  items.forEach(item => {
    initialValues[item] = false
  })
  const FormContent = ({ values }) => {
    const checkboxes = items.map(value => {
      const item = removeNonLetters(value)
      return (
...
const DeliveryPreferencesQuiz = ({ onSubmit, header, percentComplete, goBackFunction, submitText }) => {
  const FormContent = ({ values }) => {
    return (
      <>
        ...
        <CheckboxLabel htmlFor="notify_moreDeliveryDates" isChecked={values.notify_moreDeliveryDates}>
          <Checkbox type="checkbox" id="notify_moreDeliveryDates" name="notify_moreDeliveryDates"></Checkbox>
        ...
        <CheckboxLabel htmlFor="everyWeek" isChecked={values.orderFrequency === 'everyWeek'}>
          <Checkbox type="radio" id="everyWeek" name="orderFrequency" value="everyWeek"></Checkbox>
          Every Week
        </CheckboxLabel>
        <CheckboxLabel htmlFor="everyOtherWeek" isChecked={values.orderFrequency === 'everyOtherWeek'}>
          <Checkbox type="radio" id="everyOtherWeek" name="orderFrequency" value="everyOtherWeek"></Checkbox>
          Every Other Week
        </CheckboxLabel>
      </>
    )
